...
describe("SecureCirclePoly and Evaluation", () => {
  ...
  it("interpolateWithTwiddles maps columns", () => {
    ...
    const res = se.interpolateWithTwiddles({} as any);
  ...
  it("evaluateWithTwiddles delegates to polys", () => {
    ...
    const result = sp.evaluateWithTwiddles(domain, tw);
    expect(result).toBeInstanceOf(SecureEvaluation);
    expect(result.values.len()).toBe(2); // Should match domain size
