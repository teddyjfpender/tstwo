/*
This is the Rust code from vcs/blake2s_refs.rs that needs to be ported to Typescript in this vcs/blake2s_refs.ts file:
```rs
...
export const IV: Readonly<number[]> = Object.freeze([
    0x6A09E667, 0xBB67AE85, 0x3C6EF372, 0xA54FF53A, 0x510E527F, 0x9B05688C, 0x1F83D9AB, 0x5BE0CD19,
]);

export const SIGMA: Readonly<Readonly<number[]>[]> = Object.freeze([
...
const rot16 = (x: number): number => (x >>> 16) | (x << (32 - 16));

const rot12 = (x: number): number => (x >>> 12) | (x << (32 - 12));

const rot8 = (x: number): number => (x >>> 8) | (x << (32 - 8));

const rot7 = (x: number): number => (x >>> 7) | (x << (32 - 7));

export function round(v: number[], m: Readonly<number[]>, r: number): void {
    // G function (column and diagonal mixing)
    // Based on the reference Rust implementation:
    // v[0..3] are one column, v[4..7] are the next, etc.
    // Operations are done on these columns, then diagonals.

    // First part of G (columns 0 and 1 of v, SIGMA columns 0,2,4,6 and 1,3,5,7)
    // v[0] = add(v[0], m[SIGMA[r][0] as usize]);
    // v[1] = add(v[1], m[SIGMA[r][2] as usize]);
    // v[2] = add(v[2], m[SIGMA[r][4] as usize]);
    // v[3] = add(v[3], m[SIGMA[r][6] as usize]);
    v[0] = add(v[0], m[SIGMA[r][0]]);
    v[1] = add(v[1], m[SIGMA[r][2]]);
    v[2] = add(v[2], m[SIGMA[r][4]]);
    v[3] = add(v[3], m[SIGMA[r][6]]);
    ...
    // v[2] = add(v[2], m[SIGMA[r][5] as usize]);
    // v[3] = add(v[3], m[SIGMA[r][7] as usize]);
    v[0] = add(v[0], m[SIGMA[r][1]]);
    v[1] = add(v[1], m[SIGMA[r][3]]);
    v[2] = add(v[2], m[SIGMA[r][5]]);
    v[3] = add(v[3], m[SIGMA[r][7]]);
    ...
    // Second part of G (columns 0 and 1 of v, but with permuted SIGMA columns 8,10,12,14 and 9,11,13,15)
    // Also, the roles of v[0..3] and v[4..7] are somewhat swapped with v[12..15] and v[8..11]
    // and the additions are v[0]=add(v[0], v[5]) etc. instead of v[0]=add(v[0], v[4])
    
    // v[0] = add(v[0], m[SIGMA[r][8] as usize]);
    // v[1] = add(v[1], m[SIGMA[r][10] as usize]);
    // v[2] = add(v[2], m[SIGMA[r][12] as usize]);
    // v[3] = add(v[3], m[SIGMA[r][14] as usize]);
    v[0] = add(v[0], m[SIGMA[r][8]]);
    v[1] = add(v[1], m[SIGMA[r][10]]);
    v[2] = add(v[2], m[SIGMA[r][12]]);
    v[3] = add(v[3], m[SIGMA[r][14]]);
    ...
    // v[2] = add(v[2], m[SIGMA[r][13] as usize]);
    // v[3] = add(v[3], m[SIGMA[r][15] as usize]);
    v[0] = add(v[0], m[SIGMA[r][9]]);
    v[1] = add(v[1], m[SIGMA[r][11]]);
    v[2] = add(v[2], m[SIGMA[r][13]]);
    v[3] = add(v[3], m[SIGMA[r][15]]);
...
export function compress(
    h_vecs: Readonly<number[]>, // Expected length 8
    msg_vecs: Readonly<number[]>, // Expected length 16
    count_low: number,
    count_high: number,
    lastblock: number,
    lastnode: number,
): number[] { // Returns array of length 8
    const v: number[] = new Array(16);

    // Initialize v state (first 8 elements from h_vecs, next 4 from IV, last 4 from IV xored with params)
    for (let i = 0; i < 8; i++) {
        v[i] = h_vecs[i];
    }
    for (let i = 0; i < 4; i++) {
    ...
    v[12] = xor(IV[4], count_low);
    v[13] = xor(IV[5], count_high);
    v[14] = xor(IV[6], lastblock);
    v[15] = xor(IV[7], lastnode);
    ...
    // Finalization: XOR h_vecs with the first 8 words of v and the last 8 words of v
    const result: number[] = new Array(8);
    for (let i = 0; i < 8; ++i) {
        result[i] = xor(xor(h_vecs[i], v[i]), v[i + 8]);
    }
    return result;
}